...
export function LatestProgress() {
  const { data: articles, isLoading, isError, error } = useGetAllArticles();
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Recent developments and updates in the ICP ecosystem
          </p>
        </div>
        <Button
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articles.map((article) => (
            <Card
              key={article.id}
              className="bg-black/40 backdrop-blur-xl border-white/10 hover:border-purple-500/50 transition-all overflow-hidden group"
            >
              <CardContent className="p-0">
