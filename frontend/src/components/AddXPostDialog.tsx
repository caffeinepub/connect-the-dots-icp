...
import { useAddXPost } from '../hooks/useQueries';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
interface AddXPostDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function AddXPostDialog({ open, onOpenChange }: AddXPostDialogProps) {
  const [description, setDescription] = useState('');
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [uploadProgress, setUploadProgress] = useState(0);
  const addXPost = useAddXPost();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addXPost.mutateAsync({ description, image });
      ...
      <DialogContent className="bg-[#1a0a2e] border-white/10 text-foreground">
        <DialogHeader>
          <DialogTitle className="text-xl bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">
            Add X Post Screenshot
          </DialogTitle>
          <DialogDescription className="text-muted-foreground">
            Upload a screenshot of your favorite X post
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="image" className="text-white">Screenshot Image</Label>
            <Input
              id="image"
              type="file"
              accept="image/*"
