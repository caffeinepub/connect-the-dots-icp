...
import { useAddWisdom } from '../hooks/useQueries';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
interface AddWisdomDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function AddWisdomDialog({ open, onOpenChange }: AddWisdomDialogProps) {
  const [quote, setQuote] = useState('');
  const [author, setAuthor] = useState('');
  const addWisdom = useAddWisdom();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addWisdom.mutateAsync({ quote, author });
      ...
      <DialogContent className="bg-[#1a0a2e] border-white/10 text-foreground">
        <DialogHeader>
          <DialogTitle className="text-xl bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">
            Add Investing Wisdom
          </DialogTitle>
          <DialogDescription className="text-muted-foreground">
            Share a motivational quote or story
          </DialogDescription>
        </DialogHeader>
